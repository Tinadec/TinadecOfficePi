...
try {
	...
	console.warn(
		"[terminalManager] node-pty not available, falling back to child_process.spawn:",
		err.message,
	);
}
...
function getAvailableShells() {
	...
	if (platform === "win32") {
		...
		// WSL (if available)
		try {
			const wslCheck = childProcess.execSync("wsl --list --quiet", {
				encoding: "utf-8",
				timeout: 3000,
				windowsHide: true,
			});
...
function createTerminal(options = {}) {
	console.log(
		"[terminalManager] Creating terminal, usePty:",
		usePty,
		"options:",
		options,
	);
	...
	const cwd =
		options.cwd || process.env.HOME || process.env.USERPROFILE || os.homedir();
	const cols = options.cols || 80;
	const rows = options.rows || 24;
	const title = options.title || path.basename(shell);
	const env = buildEnv();

	console.log("[terminalManager] Terminal config:", {
		id,
		shell,
		args,
		cwd,
	...
	if (usePty) {
		...
		try {
			...
			console.error(
				"[terminalManager] Failed to spawn PTY, falling back to spawn:",
				err.message,
			);
...
function createSpawnFallback(entry, shell, args, cwd, cols, rows, env) {
	...
	console.log(
		"[terminalManager] Creating spawn fallback for:",
		shell,
		"on Windows:",
		isWindows,
	);
...
function writeTerminal(id, data) {
	...
	if (
		entry.isPty &&
		entry.process &&
		typeof entry.process.write === "function"
	) {
...
function resizeTerminal(id, cols, rows) {
	...
	if (
		entry.isPty &&
		entry.process &&
		typeof entry.process.resize === "function"
	) {
		try {
