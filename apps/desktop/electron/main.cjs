...
ipcMain.handle("tinadec:save-pi-model", async (_event, input) =>
	savePiModel(input),
);
ipcMain.handle("tinadec:list-pi-model-configs", () => listPiModelConfigs());
ipcMain.handle("tinadec:delete-pi-model", async (_event, input) =>
	deletePiModel(input),
);
...
ipcMain.handle("tinadec:pet-create", async (_event, petId) =>
	createPetWindow(petId),
);
ipcMain.handle("tinadec:pet-close", async (_event, instanceId) =>
	closePetWindow(instanceId),
);
ipcMain.handle("tinadec:pet-list", async () => listPetWindows());
ipcMain.handle("tinadec:pet-window-pet", async (_event, instanceId) =>
	getPetWindowPet(instanceId),
);
ipcMain.handle("tinadec:pet-current", async (event) =>
	getCurrentPetWindowPet(event.sender),
);
ipcMain.handle("tinadec:pet-current-bounds", async (event, bounds) =>
	setCurrentPetWindowBounds(event.sender, bounds),
);
ipcMain.handle("tinadec:pet-current-click-through", async (event, enabled) =>
	setCurrentPetWindowClickThrough(event.sender, enabled),
);
	...
	if (pet) {
		for (const win of BrowserWindow.getAllWindows()) {
			if (!win.isDestroyed() && win.webContents.id !== event.sender.id) {
				win.webContents.send("tinadec:pet-changed", {
					slug: pet.slug,
					enabled: pet.enabled,
				});
			}
...
ipcMain.handle("tinadec:pet-download", async (_event, slug) =>
	downloadPet(slug),
);
...
ipcMain.handle(
	"tinadec:detach-panel",
	async (_event, tabId, type, title, state) => {
...
ipcMain.handle(
	"tinadec:reattach-panel",
	async (event, tabId, type, title, state) => {
...
ipcMain.on("tinadec:close-panel-window", (_event, windowId) => {
	closePanelWindow(windowId);
});

// Focus a specific panel window by windowId
ipcMain.on("tinadec:focus-panel-window", (_event, windowId) => {
...
ipcMain.on("tinadec:broadcast-theme", (_event, theme, accentColor) => {
...
app.whenReady().then(async () => {
	...
	await Promise.all(
		downloadedPets
			...
			.map((pet) => createPetWindow(pet.slug).catch(() => undefined)),
	);
