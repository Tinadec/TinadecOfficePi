...
contextBridge.exposeInMainWorld("tinadec", {
	...
	savePiModel: (input) => ipcRenderer.invoke("tinadec:save-pi-model", input),
	listPiModelConfigs: () => ipcRenderer.invoke("tinadec:list-pi-model-configs"),
	deletePiModel: (input) =>
		ipcRenderer.invoke("tinadec:delete-pi-model", input),
	...
	pets: {
		...
		getWindowPet: (instanceId) =>
			ipcRenderer.invoke("tinadec:pet-window-pet", instanceId),
		getCurrent: () => ipcRenderer.invoke("tinadec:pet-current"),
		setCurrentBounds: (bounds) =>
			ipcRenderer.invoke("tinadec:pet-current-bounds", bounds),
		setCurrentClickThrough: (enabled) =>
			ipcRenderer.invoke("tinadec:pet-current-click-through", enabled),
		...
		fetchCatalog: (force = false) =>
			ipcRenderer.invoke("tinadec:pet-catalog", force),
		...
		setEnabled: (slug, enabled) =>
			ipcRenderer.invoke("tinadec:pet-enabled", slug, enabled),
	...
	selectBackgroundFile: (type) =>
		ipcRenderer.invoke("tinadec:select-background-file", type),
	...
	getPanelWindows: () => ipcRenderer.invoke("tinadec:get-panel-windows"),
	getCursorScreen: () => ipcRenderer.invoke("tinadec:get-cursor-screen"),
	getMainBounds: () => ipcRenderer.invoke("tinadec:get-main-bounds"),
